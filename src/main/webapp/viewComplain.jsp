...
<body>
	...
	<div class="container">
		...
		"<br>")%>
					...
					%>
					<a href="updateComplain.jsp?bbsID=<%=bbsID%>" class="btn btn-primary">수정</a>
					<a onclick="return confirm('정말로 삭제하시겠습니까?')"
						href="deleteActionComplain.jsp?bbsID=<%=bbsID%>" class="btn btn-primary">삭제</a>
					<%
					}
					%>
				
				<form action="likeActionComplain.jsp" method="post">
	...
	<script>
              document.querySelector('#likeButton').addEventListener('click', function() {
                if (this.classList.contains('liked')) {
                  ...
                  // 좋아요 추가 요청을 서버에 전송
                  var xhr = new XMLHttpRequest();
                  xhr.open('POST', 'likeActionComplain.jsp', true);
                  xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
                  xhr.onreadystatechange = function() {
                    if (xhr.readyState === 4 && xhr.status === 200) {
                      // 서버 응답이 성공적으로 완료될 경우 동작
