  ...
  <br>
			...
			<table class="table table-striped"
				...
				<tbody>
					...
					BbsDAO bbsBestDAO = new BbsDAO();
					                    ArrayList<Graduate> best_list = bbsBestDAO.Graduate_getBestList(pageNumber);
					                    int best_startNumber = (pageNumber - 1) * 10 + 1; // 시작 번호 계산
					                    for (int i = 0; i < best_list.size(); i++) {
					%>
					<tr>
						<td><a href="viewGraduate.jsp?bbsID=<%= best_list.get(i).getBbsID() %>">
	...
	<div class="container">
		<div class="row">
			<table class="table table-striped"
				...
				<tbody>
					...
					BbsDAO bbsDAO = new BbsDAO();
					                    ArrayList<Graduate> list = bbsDAO.Graduate_getList(pageNumber);
					                    int startNumber = (pageNumber - 1) * 10 + 1; // 시작 번호 계산
					                    for (int i = 0; i < list.size(); i++) {
					%>
					<tr>
						<td><%= startNumber + i %></td>
						<td><a href="viewGraduate.jsp?bbsID=<%= list.get(i).getBbsID() %>">
            ...
            if (pageNumber != 1) {
        %>
			<a href="Graduate.jsp?pageNumber=<%=pageNumber - 1%>"
        ...
        %>
			<a href="writeGraduate.jsp" class="btn btn-primary pull-right">글쓰기</a>
		</div>
	</div>
